...
class BoardOutException(BoardException):
    def __str__(self):
        return "You are trying to shoot over the board!"
...
class Game:
    ...
    def greet():
        ...
        print("-" * 55)
        print("Input format: x y; x is the row number, and y is the column number.")
        print("-" * 55)

    def print_boards(self):
        left = str(self.us.board).split("\n")
        right = str(self.ai.board).split("\n")
        print(f"{'Player board:'.center(30)}  {'Computer board:'.center(27)}")
    ...
    def loop(self):
        ...
        while True:
            self.print_boards()
            if num % 2 == 0:
                print(f'your turn!{"-"* 40}')
                repeat = self.us.move()
            else:
                print(f'Computer turn!{"-" * 43}')
                repeat = self.ai.move()
            if repeat:
                num -= 1
